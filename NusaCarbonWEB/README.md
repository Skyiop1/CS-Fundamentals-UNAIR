...
The application implements a robust **Role-Based Access Control (RBAC)** architecture and digital **Monitoring, Reporting & Verification (dMRV)** workflows.

## 📂 Repository Structure

This repository contains two main segments demonstrating the UI prototype and the fully functional backend:

- **`NusaCarbon-HTML-CSS/`** : Contains the static wireframes, mockups, and pure frontend templates built from scratch before system integration.
- **`nusacarbon-web/`** : Contains the fully functional Native PHP & MySQL Application. This is the core engine comprising the authentic blockchain simulations, database connections, and business logic.

> **Note:** All deployment and advanced functionality resides exclusively within the `nusacarbon-web` directory.
...
5. Railway's Nixpacks will detect the `composer.json`, automatically build the PHP 8.2 environment, inject `pdo_mysql` extensions, and safely expose the web application.
...
---
*Developed for Academic & Computer Science Fundamental Demonstrations @ Universitas Airlangga (UNAIR).*
